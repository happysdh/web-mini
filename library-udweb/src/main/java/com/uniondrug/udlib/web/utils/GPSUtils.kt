...
object GPSUtils {
     ...
     * @return
     */
    private fun gps84_To_Gcj02(lat: Double, lon: Double): DoubleArray {
        if (outOfChina(lat, lon)) {
            return doubleArrayOf(lat, lon)
        }
        var dLat = transformLat(lon - 105.0, lat - 35.0)
     ...
     */
    private fun gcj02_To_Gps84(lat: Double, lon: Double): DoubleArray {
     ...
     */
    private fun bd09_To_Gcj02(lat: Double, lon: Double): DoubleArray {
        val x = lon - 0.0065
        val y = lat - 0.006
        val z =
    ...
    fun isValidLatLon(text: String?): Boolean {
        return try {
            text != null && !text.isEmpty() && text.toDouble().toInt() != 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
