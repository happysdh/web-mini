package com.uniondrug.udlib.web.widget.crop;
...
public abstract class CropImageView extends AppCompatImageView {
    ...
    /******************************** 中间的FocusView绘图相关的参数 *****************************/
    public enum Style {
        RECTANGLE, CIRCLE, RECTANGLE_3_2
    }

    private Style[] styles = {Style.RECTANGLE, Style.CIRCLE, Style.RECTANGLE_3_2};
    ...
    private RectF m32FocusRect = new RectF();
    ...
    private Matrix mMatrix = new Matrix();      //图片变换的matrix
...
//        mStyle = Style.RECTANGLE_3_2;
    ...
    private void initImage() {
        ...
        m32FocusRect.left = mFocusMidPoint.x - mFocusWidth / 2;
        m32FocusRect.right = mFocusMidPoint.x - mFocusWidth / 2;
        m32FocusRect.top = mFocusMidPoint.y - mFocusHeight * (1 / 2 - 1 / 3);
        m32FocusRect.right = mFocusMidPoint.y - mFocusHeight * (1 / 2 - 1 / 3);
        ...
        //图像中点为中心进行缩放
        mMatrix.setScale(scale, scale, mImageWidth / 2, mImageHeight / 2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            canvas.drawColor(mMaskColor);
            canvas.restore();
        } else if (Style.RECTANGLE_3_2 == mStyle) {
            mFocusPath.addRect(m32FocusRect, Path.Direction.CCW);
            canvas.save();
            canvas.clipRect(0, 0, getWidth(), getHeight());
            canvas.clipPath(mFocusPath, Region.Op.DIFFERENCE);
            canvas.drawColor(mMaskColor);
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mode == DRAG) {
                    ...
                    mMatrix.postTranslate(event.getX() - pA.x, event.getY() - pA.y);
                    fixTranslation();
                    setImageMatrix(mMatrix);
                } else if (mode == ZOOM) {
                    ...
                    if (b > 10) {
                        ...
                        rotation = angleA;
                        mMatrix.set(savedMatrix);
                        mMatrix.postRotate((float) (rotation * 180 / Math.PI), midPoint.x, midPoint.y);
                        setImageMatrix(mMatrix);
                    }
                }
                break;
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (mode == DRAG) {
                    ...
                    mMatrix.set(savedMatrix);
                    mMatrix.postRotate(90 * rotateLevel, midPoint.x, midPoint.y);
...
//                    bitmap.recycle();
    ...
    public void setFocusHeight(int height) {
        mFocusHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, height, getResources().getDisplayMetrics());
        initImage();
    }
