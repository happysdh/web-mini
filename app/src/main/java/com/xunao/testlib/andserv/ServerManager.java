...
public class ServerManager extends BroadcastReceiver {
    ...
    private OnServeBack onServeBack;
    public interface OnServeBack {
    ...
    public ServerManager(Activity activity,OnServeBack serveBack ) {
        this.mActivity = activity;
        mService = new Intent(activity, CoreService.class);
        onServeBack = serveBack;
    }
        ...
        if (ACTION.equals(action)) {
            ...
            switch (cmd) {
                case CMD_VALUE_START: {
                    ...
                    if(onServeBack != null) onServeBack.onServerStart(ip);
                    break;
                }
                case CMD_VALUE_ERROR: {
                    String error = intent.getStringExtra(MESSAGE_KEY);
                    if(onServeBack != null) onServeBack.onServerError(error);
                    break;
                }
                case CMD_VALUE_STOP: {
                    if(onServeBack != null) onServeBack.onServerStop();
                    break;
                }
            }
        }
